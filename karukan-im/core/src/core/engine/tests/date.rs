...
fn committing_a_date_candidate_records_no_learning() {
    assert_date_commit_records_no_learning(false);
}

#[test]
fn typing_after_navigating_to_a_date_records_no_learning() {
    assert_date_commit_records_no_learning(true);
}

fn assert_date_commit_records_no_learning(type_after_navigation: bool) {
    ...
    engine.state = InputState::Conversion {
        ...
        reading: "きょう".to_string(),
        filter: None,
        cursor_moved: type_after_navigation,
    };

    let result = if type_after_navigation {
        engine.process_key(&press('a'))
    } else {
        engine.process_key(&press_key(Keysym::RETURN))
    };
    if type_after_navigation {
