...
macOS では `~/Library/Application Support/com.karukan.karukan-im/user_dicts/` を使用します。

- ディレクトリは自動作成されないため、初回は `mkdir -p` で作成してください
...
macOS版は「システム設定 → キーボード → ユーザ辞書」の項目も自動的に読み込みます。日本語の読みを含む項目はmacOSの統合テキスト置換データベースを読み取り専用で参照し、読み取れない場合は公開APIへフォールバックします。辞書の内容をKarukanのログやファイルへ保存することはありません。Karukanの起動時に読み込み、入力ソースが有効になったときに変更があれば再読み込みします。

ファイルで登録したKarukanユーザー辞書が先、macOSユーザ辞書が後の順にマージされ、どちらも `👤 ユーザー` 候補として表示されます。日本語の読みだけでなく、英字ショートカットもかな入力中と直接入力中の両方で候補になります。macOS日本語入力の変換学習履歴は対象外です。
